...
export function trackRoadReadyEvent(name: RoadReadyEvent, properties: Record<string, unknown> = {}) {
  if (typeof window === 'undefined') return;
  // Adapter boundary: never include form values or other personal data in event properties.
  window.dispatchEvent(new CustomEvent('roadready:analytics', { detail: { name, properties } }));
}
